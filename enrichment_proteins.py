...
def create_gene_sets(df_cis, df_trans, enr_type= ['pairs','cis_trans_set','trans_set']):
    protein_set=[]
    pair_idx =[]
    
    if enr_type == 'pairs':
        for index, row in df_trans.iterrows():
            genes_trans = [row['closest_gene_symbol']]
            seq = row['phenotype_id']
            for index_in, row_in in df_cis.iterrows():
                if row_in['phenotype_id'] == seq:
                    genes_cis = [row_in['closest_gene_symbol']]
                    gene = genes_trans + genes_cis
                    protein_set.append(gene)
                    pair_idx.append([index_in,index])
    elif enr_type == 'cis_trans_set':
        for index, row in df_cis.iterrows():
            temp = []
            genes = [row['closest_gene_symbol']]
            seq = row['phenotype_id']
            temp.append(index)
            for index_in, row_in in df_trans.iterrows():
                if row_in['phenotype_id'] == seq:
                    genes_trans = [row_in['closest_gene_symbol']]
                    genes = genes + genes_trans
                    temp.append(index_in)
            protein_set.append(genes)
            pair_idx.append(temp)
    else:
        for index, row in df_cis.iterrows():
            temp = []
            genes = []
    ...
    return protein_set, pair_idx


def create_cis_trans_df(df_full, enr_type= ['pairs','cis_trans_set','trans_set']):
    df_full['closest_gene_symbol'] = df_full['closest_gene_symbol'].astype(str)
    df_cis = df_full.loc[df_full['cis_or_trans']=='cis', ]
    df_trans = df_full.loc[df_full['cis_or_trans']=='trans', ]
    if enr_type == 'pairs':
        df_trans = df_trans.loc[df_trans['lonespot']]
    elif enr_type == 'cis_trans_set': 
        df_trans = df_trans.loc[df_trans['hotspot'] == False,]
    
    df_trans = df_trans.loc[df_trans['phenotype_id'].isin(df_cis['phenotype_id']),]
    df_trans = df_trans.reset_index(drop=True)
    df_cis = df_cis.loc[df_cis['phenotype_id'].isin(df_trans['phenotype_id']),]
    df_cis = df_cis.reset_index(drop=True)
    
    return df_cis, df_trans

def update_row(row, protein_set, pair_idx, df_cis, folds, enr_type):
    try:
        if folds is not None:
            idx = int(row['Pair']) + int(folds[int(row['Fold'])])
            pair_idx_row = pair_idx[idx]
            original_gene_set = list(protein_set[idx])
        else:
            idx = int(row['Pair'])
            pair_idx_row = pair_idx[idx]
            original_gene_set = list(protein_set[idx])

        row['original_gene_set'] = original_gene_set
        row['phenotype_id'] = df_cis.iloc[pair_idx_row[0]]['phenotype_id']
        row['UniProt_ID'] = df_cis.iloc[pair_idx_row[0]]['UniProt_ID']
        row['cis_rsid'] = df_cis.iloc[pair_idx_row[0]]['RSID']

        path_gene = row['Genes'].split(";")
        if enr_type == 'trans_set' and len(path_gene) <= 1:
            row['drop'] = True
        elif enr_type != 'trans_set':
            gene_0 = [df_cis.iloc[pair_idx_row[0]]['closest_gene_symbol']]
            found_0 = sum(x in gene_0 for x in path_gene)
            if found_0 == 0 or found_0 == len(path_gene):
                row['drop'] = True
        return row
...
# results_path = '~/Notebooks/pQTL/enrichr_results/'
# exp_name= 'pathway_enrichment_analysis_only_trans'
# enr_type= 'trans_set'
# fold =60
# enrich_genesets_dbs = 'KEGG_2021_Human'
# p_value_threshold = 0.05
...
# enriched_df = perform_enrichment_analysis( results_path=results_path , exp_name=exp_name, enr_type= enr_type,
#                                 fold = fold, enrich_genesets_dbs = enrich_genesets_dbs,
#                                 p_value_threshold = p_value_threshold, top_n_terms=top_n_terms,kegg_path = kegg_path, save_table=save_table)

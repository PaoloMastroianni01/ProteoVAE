...
def tweak_importance_massi(model, zs, factor, n_clusters=4, L=100, M=1000):
    ...
    for m in tqdm(range(M)):
        zsh = zs[np.random.permutation(zs.shape[0])][:L]
        zsh = np.concatenate([zsh, np.zeros((zsh.shape[0], 1))], axis=1)  # add dummy column for conditioning
        print(f"m: {m}, zsh.shape: {zsh.shape}")
        # duplicate, sign change on guided latent, decode & diff
        hsz = np.copy(zsh)
        ...
        xs = model.decoder(torch.tensor(zsh,dtype=torch.float32))
        sx = model.decoder(torch.tensor(hsz,dtype=torch.float32))
        diff = torch.abs(
